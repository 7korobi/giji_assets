...
function initFilter() {
  ...
  if (cookie.clipboard) $('#clipboard').val(unescape(cookie.clipboard))
  if (cookie.cliptext) $('#cliptext').val(unescape(cookie.cliptext))
  ...
  if (cookie.fixedfilter == 1 && cookie.layoutfilter == 1) fixFilterNoWriteCookie()
  return
}
...
function writeCookieFilter() {
  ...
  for (var key in toggle) {
    if (toggle[key] == 1) cookie.toggle += key + ','
  }

  // クッキーの記入

  for (var key in cookie) {
    if (!cookie_in_use[key]) continue
    if (filter[key]) {
      expires = deletes
    } else {
      expires = alives
...
function changeSayDisplays() {
  ...
  $('.message_filter').each(function () {
    ...
    if (filter['pnofilter_' + logpno] == 1 || filter['typefilter_' + mestype] == 1) {
      display = 'none'
    } else {
  ...
  $('.message_filter').each(function () {
    ...
    if (cliptext) {
      ...
      for (var i = 0; i < clipary.length; i++) {
        if (-1 < text.indexOf(clipary[i])) item += ' ' + clipary[i]
      }
    }
    ...
    result_list.push(
      '<div class="sayfilter_content_enable"><div class="sayfilter_incontent"><a class="res_anchor" href="#' +
        link +
        '">' +
        name +
        '</a>' +
...
function deployToolTip() {
  ...
  navimode_fullwidth = navimode.find(
    'h2,.turnnavi,.row_all,.mes_admin,.mes_maker,.info,.infosp,.infowolf,.caution,.action_bm'
  )
