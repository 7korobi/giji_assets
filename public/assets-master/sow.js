...
// Generated by LiveScript 1.3.1
(function(){
  var sow, out$ = typeof exports != 'undefined' && exports || this;
  out$.sow = sow = {
    auth: {}
  };
}).call(this);
...
GUI.if_exist("#css_changer", function(dom) {
  return m.module(dom, {
    controller: function() {},
    view: function() {
      return m(".guide", m("a.menuicon.pull-right.icon-cog", menu.icon.start({}, "cog"), " "), sow.auth.is_login ? m("a.btn.edge[href=" + gon.url + "?ua=mb&cmd=vindex&uid=" + (sow.auth.uid()) + "&pwd=" + (sow.auth.pwd()) + "]", "携帯") : m("a.btn.edge[href=" + gon.url + "?ua=mb]", "携帯"), Btns.radio({}, Url.prop.theme, {
        cinema: "煉瓦",
        star: "蒼穹",
        night: "闇夜",
        moon: "月夜",
...
if ((typeof gon !== "undefined" && gon !== null ? gon.sow_auth : void 0) != null) {
  GUI.if_exist("#buttons", function(dom) {
    catch_gon.sow_auth();
    return menu.icon.icon("pin", {
      open: function() {},
      close: function() {},
      view: function() {
        return [m("label", m("span.mark.pull-left", "user id : "), m("input.mini", Txt.input(sow.auth.uid))), m("label", m("span.mark.pull-left", "password : "), m("input.mini[type=password]", Txt.input(sow.auth.pwd))), m("hr.black")];
      }
    });
  });
}
...
if ((typeof gon !== "undefined" && gon !== null ? gon.items : void 0) != null) {
  ...
  items_module = function(type) {
    return GUI.if_exist("#" + type, function(dom) {
      var query;
      query = Cache.items.where({
        type: type
      });
      return m.module(dom, {
        controller: function() {},
        view: function() {
          return query.list().map(function(v) {
            return GUI.message[v.template](v);
          });
        }
      });
    });
